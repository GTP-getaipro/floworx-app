...
const createWorkflowSchema = Joi.object({
  ...
  businessType: Joi.string()
    .valid('hot_tub', 'pool', 'spa', 'wellness', 'hospitality', 'other')
    .required()
    .messages({
  ...
  triggers: Joi.array()
    .items(
      Joi.object({
        type: Joi.string()
          .valid('email', 'webhook', 'schedule', 'manual')
          .required()
          .messages({
...
const workflowTemplateSchema = Joi.object({
  ...
  businessTypes: Joi.array()
    .items(
      Joi.string().valid('hot_tub', 'pool', 'spa', 'wellness', 'hospitality', 'other')
    )
    .min(1)
    .required()
    .messages({
...
const workflowSearchSchema = Joi.object({
  ...
  status: Joi.string()
    .valid('active', 'inactive', 'draft', 'error')
    .optional()
    .messages({
...
const workflowExecutionHistorySchema = Joi.object({
  ...
  status: Joi.string()
    .valid('running', 'success', 'error', 'waiting', 'canceled')
    .optional()
    .messages({
  ...
  triggeredBy: Joi.string()
    .valid('email', 'webhook', 'schedule', 'manual', 'api')
    .optional()
    .messages({
...
const webhookConfigSchema = Joi.object({
  ...
  path: Joi.string()
    ...
    .required()
    .messages({
      'string.pattern.base': 'Webhook path must start with / and contain only alphanumeric characters, hyphens, underscores, and forward slashes',
      'string.empty': 'Webhook path is required',
      'any.required': 'Webhook path is required'
    }),
  authentication: Joi.object({
    type: Joi.string()
      .valid('none', 'basic', 'bearer', 'api_key', 'signature')
      .default('none')
      .messages({
