...
const ForgotPassword = () => {
  ...
  const checkAccountLockout = useCallback(async () => {
    try {
      const response = await axios.post(`${process.env.REACT_APP_API_URL}/account-recovery/check-lockout`, {
        ...
        <form onSubmit={handleSubmit} className="auth-form">
          ...
          {lockoutInfo && lockoutInfo.locked && (
            <Alert variant="warning" className="mb-4">
              <strong>Account Locked:</strong> Your account is temporarily locked due to multiple failed login attempts.
              It will be unlocked in {lockoutInfo.remainingTime} minutes, or you can use account recovery options below.
            </Alert>
          )}
