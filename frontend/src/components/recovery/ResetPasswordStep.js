...
const ResetPasswordStep = ({ recoveryData, onComplete, onError }) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <Alert variant="info">
        <strong>Security Tip:</strong> Use a unique password that you haven't used elsewhere. Consider using a password manager to generate and store strong passwords.
      </Alert>
          ...
          <div className="bg-surface-subtle rounded-lg p-4">
            ...
            <ul className="space-y-1">
              {passwordRequirements.map((requirement, index) => {
                const isMet = formData.newPassword && validatePassword(formData.newPassword).length === 0;
                return (
      ...
      {passwordStrength > 0 && passwordStrength < 60 && (
        <Alert variant="warning">
          Your password strength is {getStrengthText(passwordStrength).toLowerCase()}. Consider adding more characters, numbers, or special characters to make it stronger.
        </Alert>
      )}
    </div>
  );
