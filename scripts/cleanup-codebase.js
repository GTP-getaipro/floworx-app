...
const CLEANUP_CONFIG = {
  // Files to remove (duplicates in backup-imports)
  filesToRemove: [
    'backend/backup-imports',
    'backend/coverage', // Regenerated by tests
    'frontend/build', // Regenerated by build
  ...
  importFixes: [
    {
      file: 'backend/scripts/quality-check.js',
      line: 11,
      fix: 'const colors = require(\'colors\');',
      issue: 'Missing colors import'
    },
    {
      file: 'backend/routes/analytics.js',
      line: 1,
      fix: 'const { authenticateToken } = require(\'../middleware/auth\');',
      issue: 'Missing auth middleware import'
    }
  ],
...
async function runCleanup() {
  console.log('🧹 Starting FloWorx Codebase Cleanup...');
  console.log('=====================================\n');
  ...
  try {
    // Step 1: Remove duplicate and unnecessary files
  ...
  for (const fix of CLEANUP_CONFIG.importFixes) {
    try {
      if (fs.existsSync(fix.file)) {
        ...
        if (fix.issue === 'Missing colors import' && content.includes('// const colors = require(\'colors\'); // Unused - removed')) {
          lines[fix.line - 1] = fix.fix;
          content = lines.join('\n');
          fs.writeFileSync(fix.file, content);
          console.log(`  ✅ Fixed import in: ${fix.file}`);
          results.importsFixed++;
        }
      }
    } catch (error) {
