...
router.get('/google/callback', async (req, res) => {
  try {
    ...
    await pool.query(upsertCredentialQuery, [
      userId,
      'google',
...
router.delete('/google', authenticateToken, async (req, res) => {
  try {
    ...
    const result = await pool.query(deleteQuery, [req.user.id, 'google']);
...
const refreshGoogleToken = async (userId) => {
  try {
    ...
    await pool.query(updateQuery, [encryptedAccessToken, expiryDate, userId, 'google']);
