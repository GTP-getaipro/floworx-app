import React, { useState, useEffect } from 'react';

const Toast = ({
  children,
  variant = 'info',
  duration = 5000,
  onClose,
  className = '',
  ...props
}) => {
  const [isVisible, setIsVisible] = useState(true);

  useEffect(() => {
    if (duration > 0) {
      const timer = setTimeout(() => {
        setIsVisible(false);
        if (onClose) onClose();
      }, duration);

      return () => clearTimeout(timer);
    }
  }, [duration, onClose]);

  const variants = {
    success: {
      container: 'bg-surface border-l-4 border-success shadow-card',
      icon: (
        <svg className='w-5 h-5 text-success' fill='currentColor' viewBox='0 0 20 20'>
          <path
            fillRule='evenodd'
            d='M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z'
            clipRule='evenodd'
          />
        </svg>
      ),
    },
    warning: {
      container: 'bg-surface border-l-4 border-warning shadow-card',
      icon: (
        <svg className='w-5 h-5 text-warning' fill='currentColor' viewBox='0 0 20 20'>
          <path
            fillRule='evenodd'
            d='M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z'
            clipRule='evenodd'
          />
        </svg>
      ),
    },
    danger: {
      container: 'bg-surface border-l-4 border-danger shadow-card',
      icon: (
        <svg className='w-5 h-5 text-danger' fill='currentColor' viewBox='0 0 20 20'>
          <path
            fillRule='evenodd'
            d='M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z'
            clipRule='evenodd'
          />
        </svg>
      ),
    },
    info: {
      container: 'bg-surface border-l-4 border-info shadow-card',
      icon: (
        <svg className='w-5 h-5 text-info' fill='currentColor' viewBox='0 0 20 20'>
          <path
            fillRule='evenodd'
            d='M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z'
            clipRule='evenodd'
          />
        </svg>
      ),
    },
  };

  if (!isVisible) return null;

  const variant_config = variants[variant];

  return (
    <div
      className={`
        fixed top-4 right-4 z-50 max-w-xs sm:max-w-md lg:max-w-lg w-full
        ${variant_config.container}
        rounded-lg p-4 transition-all duration-300 ease-in-out
        ${className}
      `
        .replace(/\s+/g, ' ')
        .trim()}
      {...props}
    >
      <div className='flex items-start'>
        <div className='flex-shrink-0'>{variant_config.icon}</div>
        <div className='ml-3 flex-1 min-w-0'>
          <div className='text-sm text-ink break-words whitespace-pre-wrap'>{children}</div>
        </div>
        {onClose && (
          <div className='ml-4 flex-shrink-0'>
            <button
              className='inline-flex text-ink-sub hover:text-ink focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-primary-hover rounded-sm'
              onClick={() => {
                setIsVisible(false);
                onClose();
              }}
            >
              <span className='sr-only'>Close</span>
              <svg className='w-5 h-5' fill='currentColor' viewBox='0 0 20 20'>
                <path
                  fillRule='evenodd'
                  d='M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z'
                  clipRule='evenodd'
                />
              </svg>
            </button>
          </div>
        )}
      </div>
    </div>
  );
};

export default Toast;
