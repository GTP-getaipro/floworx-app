-- Floworx Email Provider Selection Migration
-- Run this to add email provider selection and user configurations support
-- This extends the existing onboarding system with email provider selection
...
-- RLS Policy for user_configurations (users can only access their own configuration)
...
BEGIN
    ...
    IF EXISTS (
        ...
        RAISE NOTICE '✅ email_provider column added to users table';
    ELSE
        RAISE EXCEPTION '❌ Failed to add email_provider column to users table';
    END IF;
    ...
    IF EXISTS (
        ...
        RAISE NOTICE '✅ user_configurations table created';
    ELSE
        RAISE EXCEPTION '❌ Failed to create user_configurations table';
    END IF;
    ...
    IF EXISTS (
        ...
        RAISE NOTICE '✅ RLS enabled on user_configurations table';
    ELSE
        RAISE NOTICE '⚠️  RLS may not be enabled on user_configurations table';
    END IF;
