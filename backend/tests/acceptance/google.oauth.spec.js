...
describe('Google OAuth Integration', () => {
  ...
  beforeEach(async () => {
    ...
    const testUser = {
      email: `google-oauth-test-${Date.now()}@example.com`,
  ...
  describe('GET /api/integrations/google/authorize', () => {
    it('should return authorization URL with correct parameters', async () => {
      ...
      expect(params.get('redirect_uri')).toBe(process.env.GOOGLE_REDIRECT_URI);
  ...
  describe('GET /api/integrations/google/callback', () => {
    ...
    it('should handle successful OAuth callback', async () => {
      ...
      expect(response.headers.location).toContain('/onboarding/step2?connected=gmail');
      ...
      expect(connection.sub).toBe('test@gmail.com');
      ...
      expect(onboardingState.rows[0].data.gmailConnected).toBe(true);
    });
  ...
  describe('POST /api/integrations/google/disconnect', () => {
    ...
    beforeEach(async () => {
      ...
      `, [
        userId, 'google', connectionData.sub,
      ...
      await query(`
        ...
        SET data = jsonb_set(COALESCE(data, '{}'), '{gmailConnected}', 'true')
        WHERE user_id = $1
      `, [userId]);
    });
    ...
    it('should disconnect Google account successfully', async () => {
      ...
      expect(onboardingState.rows[0].data.gmailConnected).toBe(false);
    });
