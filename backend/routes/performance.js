...
router.get('/metrics', authenticateToken, asyncWrapper(async (req, res) => {
...
router.get('/endpoints', authenticateToken, asyncWrapper(async (req, res) => {
...
router.get('/database', authenticateToken, asyncWrapper(async (req, res) => {
  const summary = performanceService.getPerformanceSummary();
...
router.get('/cache', authenticateToken, asyncWrapper(async (req, res) => {
...
router.post('/cache/clear', authenticateToken, asyncWrapper(async (req, res) => {
...
router.get('/recommendations', authenticateToken, asyncWrapper(async (req, res) => {
...
router.post('/audit', authenticateToken, asyncWrapper(async (req, res) => {
  // Check if user is admin
  // if (!req.user.isAdmin) {
  //   return res.status(403).json({ error: 'Admin access required' });
  // }
...
router.get('/system', authenticateToken, asyncWrapper(async (req, res) => {
...
router.post('/reset', authenticateToken, asyncWrapper(async (req, res) => {
  // Check if user is admin
  // if (!req.user.isAdmin) {
  //   return res.status(403).json({ error: 'Admin access required' });
  // }
...
router.get('/dashboard', authenticateToken, asyncWrapper(async (req, res) => {
  ...
  const dashboard = {
    ...
    performance: {
      ...
      averageResponseTime: summary.slowRequests.length > 0 ? 
        Math.round(summary.slowRequests.reduce((sum, req) => sum + req.avgTime, 0) / summary.slowRequests.length) : 0
    },
    cache: {
